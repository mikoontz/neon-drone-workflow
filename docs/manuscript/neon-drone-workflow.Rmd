...
We used the `ForestTools` package to implement this algorithm and used the optimal tree detection parameter set determined by @young2021, with a variable search window defined using the following function: 
...
We used the `NeonTreeEvaluation` package


`neonUtilities` [@lunch2021], `neonhs` [@joseph2021a], `geoNEON` [@nationalecologicalobservatorynetwork2020], and `NeonTreeEvaluation` [@weinstein2021a].
...
For the NEON-derived NDVI, the constituent red and near infrared reflectance are spectrally resampled in order to match the red and near infrared spectral response of the MicaSense RedEdge 3 sensor.

lidR [@roussel2020; @roussel2021], ForestTools [@plowright2021]
