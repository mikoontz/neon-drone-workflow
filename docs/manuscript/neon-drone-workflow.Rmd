...
It is best practice to adhere as close as possible to the "3-2-1 Backup Rule", where three copies of the data exist with a local, accessible copy on two different devices (e.g., local computer and an external hard drive) and one copy off-site (e.g., a cloud backup service) [@ruggiero2012]. 
...
Once the data collection is completed, data management can be broken into a quick access phase and a slower access phase based on how readily available they need to be.
The first phase immediately post data collection while data are being processed
The second phase deals with longer-term storage of both the data and metadata.

For data processing, it 

For such high volumes of data, it is sound data management practice to establish "data levels" that characterize how derived each new processed product is, with Level 0 representing raw data and higher levels being derived from lower levels [@wyngaard2019; @koontz2021].
...
as well as a laptop and an external hard drive(s) to transfer the data immediately, and back them up.
