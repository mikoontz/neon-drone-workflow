...
In other cases, the payload relies both on power and electronic signaling from the vehicle in order to capture data, and integration may require more specialized electrical engineering expertise.
...
Environmental conditions may also impact data collection on automated flights.
For optical data, ideal conditions are evenly lit with either complete cloud cover or clear skies.
...
It’s important to bring empty SD cards (with fast write speed, if possible) into the field to swap out of the UAS for frequent data transfer, as well as a laptop and an external hard drive(s) to transfer the data immediately, and back them up.
