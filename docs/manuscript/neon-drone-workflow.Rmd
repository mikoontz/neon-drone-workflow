...
When working with optical data, it may be necessary to "spectrally resample" the high spectral resolution NEON AOP in order to match the sensor payload of the UAS, whose spectral resolution is likely coarser .
...
`neonUtilities` [@lunch2021], , `geoNEON` [@nationalecologicalobservatorynetwork2020], and `NeonTreeEvaluation` .

To integrate our UAS data with NEON AOP reflectance data, we calculated NDVI from each sensor.
We used the `neonUtilities` package to download the NEON AOP imaging spectrometer data (data product DP3.30006.001) that covers the NIWO_017 site from 2019, using the easting and northing of the centroid of the NIWO_017 plot and a 20 m buffer to search with the `byTileAOP()` function [@lunch2021].
