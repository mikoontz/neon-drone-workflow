...
Specifically, we focus on optical data collected by each tool over terrestrial sites and provide (1) a collection of what we consider to be the nine most important practical considerations for collecting high-quality optical data from a UAS for integration with NEON (science requirements, vehicle, payload, environment, flight planning, rules/regulations, radiometric calibration, georeferencing, data management, and data processing) and (2) a real-world workflow illustrating how to 
consider these depicting the processing of UAS data into meaningful ecological information at the NEON Niwot Ridge (NIWO) field site.
...
In light of this, we echo @assmann2019 in emphasizing that the main consideration to keep in mind while when integrating UAS with NEON is: what are the science requirements for the data to be collected and what data collection efforts are "good enough" to meet those requirements?
...
vehicle_payload_table_print <-
  tibble::tibble(Consideration = c("", "", "", "Vehicle", "", "", "",
                 ...
                 Advantages = c("Ease of takeoff and landing",
                                "Hover capability",
                                "maneuverability",
                                "Affordable",
...
Unlike permissions granteed under a COA, the Part 107 credentials stay with the pilot and are transferable if the pilot changes organizations (e.g., a graduate student can't operate a UAS for research under a university's COA after they graduate, but they would still retain their ability to operate under Part 107).
...
This change in reflectance demonstrates the necessity to clean, re-measure, or replace calibration panels when performing radiometric calibration.")
```
...
Many free, open source software tools exist for working with SfM data products including QGIS for visualization and GUI-based geospatial manipulation of raster and vector data types, CloudCompare for visualization and GUI-based manipulation of point clouds, and a suite of packages [@atkins2022] for the R programming language [@rcoreteam2021].

neonhs [@joseph2021a], lidR [@roussel2020; @roussel2021], ForestTools [@plowright2021], hsdar [@atkins2022]
...
Map Pilot is a reliable, full-featured flight planning application that allows us to set 
...
We flew under the FAA Part 107 rules for commercial drone operations, which isa broadly permissive set of regulations for capturing optical data.

The airspace over NIWO is Class G uncontrolled up to 1,200 feet above ground level.

### Radiometric Calibration

The Micasense RedEdge3 multispectral camera comes with a small, grey calibrated reflectance panel that reflects approximately 60% of light across the entire spectral extent captured by the sensor.
...
The RedEdge3 also integrates a DLS, which faces upwards and measures illumination at the same time as the downward-facing image capture.
...
When including the calibration panel photos, we set the "known reflectance" of the panel in each of the five spectral channels to be those that we measured for
...
We followed the USGS workflow to process our raw Micasense RedEdge 3 imagery into an orthomosaic and a dense point cloud [@over2021].
...
We used a local maximum filter to detect individual trees [@popescu2004] as implemented in the `lidR` package and a marker controlled watershed segmentation algorithm to segment individual tree crowns [@plowright2021].
We used the `hsdar` package to spectrally resample the NEON imaging spectrometer data to match the spectral resolution of the Micasense RedEdge3 camera [@lehnert2019].
...
For the NEON-derived NDVI, the constituent red and near infrared reflectance are spectrally resampled in order to match the red and near infrared spectral response of the Micasense RedEdge3 sensor.
...
1. Phenology of individual trees linked to phenocam and multiple, frequent UAS flights
1. Predicting snow retention by enhancing NEON forest structure/topography measurements with UAS finer-scale measurements
